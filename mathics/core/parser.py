...
class ScanError(TranslateError):
    ...
    def __unicode__(self):
        return "Lexical error at position {0}.".format(self.pos)
...
symbol_re = re.compile(r'`?{0}(`{0})*'.format(base_symb))
...
class MathicsScanner:
    ...
    def t_number(self, t):
        ...
        if base == 10:
            if n != 0:
                s = "{0}E{1}".format(s, n)    # sympy handles this
    ...
    @lex.TOKEN(r'{0}?_\.'.format(base_symb))
    def t_blankdefault(self, t):    # this must come before t_blanks
        # r' ([a-zA-Z$][a-zA-Z0-9$]*)?_\. '
        return t

    @lex.TOKEN(r'{0}?_(__?)?{0}?'.format(base_symb))
    def t_blanks(self, t):
        # r' ([a-zA-Z$][a-zA-Z0-9$]*)?_(__?)?([a-zA-Z$][a-zA-Z0-9$]*)? '
        return t

    @lex.TOKEN(r'`?{0}(`{0})*'.format(base_symb))
    def t_symbol(self, t):
